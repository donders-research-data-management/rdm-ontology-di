...
if __name__ == "__main__":
    ...
    parg = argparse.ArgumentParser(description='check availability of external URLs defined by an index file (e.g. external_urls.json) within the release zipfile')
    ...
    try:
        ...
        for k, v in sorted(d_urls.items(), key=operator.itemgetter(1)):
            ...
            if re.match('.*{.*.}.*', v ):
                logger.warning("file or dir for {}: SKIPPED".format(v))
                continue
