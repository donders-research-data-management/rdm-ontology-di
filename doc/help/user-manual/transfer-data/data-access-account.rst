...
.. note::
    ...
    If you authenticate yourself on the portal (https://${repositoryUrl}) with credentials provided by the Radboud University or RadboudUMC, you do *not* require the one-time access password. Instead, you can use the credentials and password provided by the Radboud University or RadboudUMC to be able to upload, download or view content of files.

To get the data access password you first must login (Read: :ref:`How to login? <login-portal>`) in the portal (https://${repositoryUrl}). After logging in, click on your display name at the top-right corner to open a drop-down menu, and select *Data Access Password*.
